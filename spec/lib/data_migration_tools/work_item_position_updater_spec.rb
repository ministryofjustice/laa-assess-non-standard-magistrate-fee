...
RSpec.describe DataMigrationTools::WorkItemPositionUpdater do
  ...
  let(:claim) { create(:claim, data: { work_items: }) }

  context 'when work_items without positions exist on claim' do
    let(:work_items) do
      [
        build(:work_item, id: 'bbb222', position: nil, completed_on: 2.days.ago.to_date.iso8601,
              work_type: { 'value' => 'waiting', 'en' => 'Waiting' }),
        build(:work_item, id: 'bbb111', position: nil, completed_on: 3.days.ago.to_date.iso8601,
              work_type: { 'value' => 'travel', 'en' => 'Travel' }),
        build(:work_item, id: 'aaa111', position: nil, completed_on: 3.days.ago.to_date.iso8601,
              work_type: { 'value' => 'attendance_with_counsel', 'en' => 'Attendance with counsel' }),
      ]
    end

    it 'updates existing work items with sorted position value' do
      expect { updater.call }
        .to change { claim.reload.data['work_items'] }
        .from(
          [
  ...
  context 'when an error is encountered' do
    ...
    before do
      allow(claim).to receive(:save!).and_raise StandardError, 'oops, something went wrong!'
