...
RSpec.describe 'History events' do
  ...
  it 'shows all (visible) events in the history' do
    ...
    Event::NewVersion.build(claim: claim)
    ...
    Event::Assignment.build(claim: claim, current_user: caseworker)
    Event::ChangeRisk.build(claim: claim, explanation: 'Risk change test', previous_risk_level: 'high', current_user: caseworker)
    Event::Note.build(claim: claim, current_user: caseworker, note: 'User test note')
    claim.state = 'further_info'
    Event::SendBack.build(claim: claim, current_user: caseworker, previous_state: 'submitted', comment: 'Send Back test')
    claim.state = 'granted'
    Event::Decision.build(claim: claim, current_user: caseworker, previous_state: 'further_info', comment: 'Decision test')
