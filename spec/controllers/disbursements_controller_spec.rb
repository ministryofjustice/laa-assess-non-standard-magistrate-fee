...
RSpec.describe DisbursementsController do
  ...
  context 'edit' do
    ...
    let(:disbursement) { instance_double(V1::Disbursement, id: disbursement_id, attributes: item, form_attributes: {}) }
    let(:item) do
      {
        'id' => disbursement_id,
        'miles' => nil,
  ...
  context 'update' do
    ...
    context 'when form save is successful' do
      ...
      it 'renders successfully with claims' do
        ...
        expect(controller).to redirect_to(claim_adjustments_path(claim, anchor: 'disbursements-tab'))
