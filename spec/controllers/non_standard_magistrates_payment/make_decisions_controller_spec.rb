...
RSpec.describe NonStandardMagistratesPayment::MakeDecisionsController do
  context 'edit' do
    ...
    let(:decision) { instance_double(NonStandardMagistratesPayment::MakeDecisionForm) }

    before do
      allow(Claim).to receive(:find).and_return(claim)
      allow(NonStandardMagistratesPayment::MakeDecisionForm).to receive(:new).and_return(decision)
    end
  ...
  context 'update' do
    ...
    it 'builds a decision object' do
      put :update, params: {
        claim_id: claim.id,
        non_standard_magistrates_payment_make_decision_form: { state: 'granted', partial_comment: nil,
reject_comment: nil, id: claim.id }
      }
      expect(NonStandardMagistratesPayment::MakeDecisionForm).to have_received(:new).with(
